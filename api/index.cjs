...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', message: 'Evidence storage server running on Vercel' });
});
...
app.post('/api/upload-evidence', upload.single('file'), async (req, res) => {
  try {
    ...
    // We can use pinJSONToIPFS for the metadata and pinFileToIPFS for the buffer
    // But since PinataSDK pinFileToIPFS expects a stream or path, we can create a readable stream from the buffer
